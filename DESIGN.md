...
**Subject-context predicates** (landed 2026-07-07; owner-named consumer: sex/age reference ranges — "una cosa tipica della biologia con valori uomo donna diversi e che variano con l'età"). A fixed `gt`/`lt` cutoff cannot express anaemia, whose threshold depends on the patient: Hb < 12 g/dL in women, < 13 g/dL in men. The generalisation is `analyte_value(codes, keep_when = <closure>)` — a plain vectorised row predicate whose **formals name raw columns** on the analyte's rows (the measured `value` plus any subject attribute the source carries):

``` r
hb_low = lab_channel(
  source   = "biology",
  selector = analyte_value("HGB", keep_when = \(value, PATSEX) value < ifelse(PATSEX == "F", 12, 13))
)
```

This is the [reducers-are-plain-functions] rule extended to *membership*: the researcher's closure **is** the rule, not a sex-keyed threshold table the engine would interpret (the wrapper razor rejects that). The formal-names-a-column convention is the same explicit-column choice ratified for `index_event(at =)`; only columns **declared on the `source_spec`** survive normalization, so a subject attribute must be declared (role-less is fine — `PATSEX`/`PATAGE` on `BIOL_SOURCE`, the pattern of docs' `SEJUM`/`SEJUF`) to be visible to the predicate. The closure is applied only to the analyte-matched rows (so `value` is always that analyte's measurement); an `NA` result is not a hit, exactly like the fixed bound. `gt`/`lt` remain the fixed-cutoff sugar and are **mutually exclusive** with `keep_when` (fold any fixed part into the closure); a formal naming an uncarried column, or a result of the wrong shape/type, is a hard error (the same contract discipline as `keep_group_when` and a payload `reduce`).
