...
| Source registry and roles | Local source specs still use migration-era role names such as `subject`, `event`, `record`, `interval_start`, `interval_end`, `value`, `value_raw`, and `native_ref`. | `source_spec()` is seeded from `redsan::edsan_sources()` and exposes an open canonical role set: identifier spine, point/interval time roles, query/batch date keys, payload roles, and auxiliary row coordinates. | Next slice: add target role names and source metadata without treating the role list as exhaustive; preserve loader compatibility. |
