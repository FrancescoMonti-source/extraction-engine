...
| Text method | `llm_after_lucene(top_n=...)`; `top_n` is set-but-not-read (text is pre-retrieved). | `llm_after_lucene(candidates = llm_candidate_selection(arrange=..., limit=...), prompt=..., type=..., positive_hit_when=...)` | Selection helper named `llm_candidate_selection()` (disambiguated from the structured `candidates` frame). Deferred until retrieval runs in-engine and there is a consumer; do not add a second unread seam before then. |
