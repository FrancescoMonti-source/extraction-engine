...
suppressWarnings(suppressMessages({
    ...
    ) # attempts -> UTF-8+BOM csv
}))

# ---- config -----------------------------------------------------------------
DATASETS <- Sys.getenv("DATASETS_DIR", "C:/Users/franc/Documents/Datasets")
POOL <- file.path(DATASETS, "tabac_eval_pool_1000.rds")
MODELS <- strsplit(Sys.getenv("SMOKE_MODELS", "gemma3:4b"), ",")[[1]] # ellmer strips :latest -> use "gemma4" not "gemma4:latest"
...
run_one <- function(chat, type, text) {
    ...
    out$latency_ms <- as.numeric(difftime(Sys.time(), t0, units = "secs")) *
        1000
    out
}
...
validate <- function(res, source_text) {
    ...
    st <- res[["smoking_status"]]
    ev <- res[["evidence"]]
    v$schema_ok <- is.character(st) &&
        length(st) == 1 &&
        (is.null(ev) || is.character(ev))
    if (!isTRUE(v$schema_ok)) {
        return(v)
    }
    v$status <- st
...
tert <- cut(
    len,
    breaks = quantile(len, c(0, 1 / 3, 2 / 3, 1), na.rm = TRUE),
    include.lowest = TRUE,
    labels = c("short", "mid", "long")
)
per <- max(1L, N %/% 3L)
idx <- unlist(lapply(split(d$.row_id, tert), function(ids) {
    sample(ids, min(per, length(ids)))
}))
idx <- head(idx, N)
samp <- d[idx, ]

cat(sprintf(
    "Phase 0 smoke test | pool=%d rows | sample=%d | num_ctx=%d | max_tokens=%d | seed=%d\n",
    nrow(d),
    nrow(samp),
    NUM_CTX,
    MAX_TOKENS,
...
type <- tryCatch(
    ellmer::type_from_schema(text = SCHEMA_JSON),
    error = function(e) {
        cat(
            "FATAL: type_from_schema() rejected the spec:\n  ",
            conditionMessage(e),
            "\n"
        )
        quit(status = 1)
...
for (model in MODELS) {
    chat <- tryCatch(make_chat(model), error = function(e) {
        cat(sprintf(
            "[%s] could not init chat: %s\n",
            model,
            conditionMessage(e)
        ))
        NULL
    ...
    for (i in seq_len(nrow(samp))) {
        ...
        v <- if (identical(out$status, "ok")) {
            validate(out$res, r$text_tabac_llm)
        } else {
            validate(NULL, r$text_tabac_llm)
        }
        ...
        attempts[[length(attempts) + 1]] <- data.frame(
            ...
            row_id = r$.row_id,
            status = out$status,
            latency_ms = round(out$latency_ms),
            error = out$error,
        ...
        rows[[length(rows) + 1]] <- data.frame(
            ...
            row_id = r$.row_id,
            role = r$role,
            n_chars = nchar(r$text_tabac_llm),
            call_status = out$status,
            schema_ok = v$schema_ok,
            status = v$status,
            status_in_enum = v$status_in_enum,
            is_not_stated = v$is_not_stated,
            evidence_exact = v$evidence_exact,
            evidence_norm = v$evidence_norm,
            latency_ms = round(out$latency_ms),
            evidence = if (identical(out$status, "ok") && is.list(out$res)) {
                (if (is.null(out$res[["evidence"]])) {
                    ""
                } else {
                    out$res[["evidence"]]
                })
...
writexl::write_xlsx(
    res_df,
    file.path(OUT_DIR, sprintf("smoke_rows_%s.xlsx", stamp))
)
# Attempt log as UTF-8 + BOM csv (write_excel_csv) so Excel detects the encoding.
readr::write_excel_csv(
    att_df,
    file.path(OUT_DIR, sprintf("smoke_attempts_%s.csv", stamp))
)
...
for (model in unique(res_df$model)) {
    ...
    cat(sprintf(
        "  2. schema valid ......... %d/%d (%s)\n",
        sum(m$schema_ok, na.rm = TRUE),
        n,
        pct(m$schema_ok)
    ))
    cat(sprintf("     status in enum ....... %s\n", pct(m$status_in_enum)))
    enum_tab <- table(factor(m$status, levels = ENUM))
    cat("  3. status distribution ... ")
    cat(
        paste(sprintf("%s=%d", names(enum_tab), enum_tab), collapse = "  "),
        "\n"
    )
    ndecided <- m[!is.na(m$evidence_exact), ]
    cat(sprintf(
        "  4. evidence substring .... exact %s | normalized %s  (of %d decided rows)\n",
        pct(ndecided$evidence_exact),
        pct(ndecided$evidence_norm),
        nrow(ndecided)
    ))
