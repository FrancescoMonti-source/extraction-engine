...
The contract above is the **settled target** (2026-06-26). **Slice 2 has landed** (branch `codex/multisource-diabetes-spike`): `any_positive()` now lowers to a hit-set expression at `variable_spec()` build; `documented_status()` / `collect_fields()` are gone as combiners — single-channel assembly is dispatched on `output` shape with `combine = NULL`, making `is.null(combine)` the sole single-vs-multi discriminator; and the validity matrix below is enforced in the constructor. A boolean variable's `value` is now always `0`/`1` (the open-world NA moved onto `channel_coverage`). `combine = any_positive()` stays valid as the sugar spelling. **Slice 3 has also landed**: `combine_rule` now stores the raw expression (e.g. `"pmsi_diag_dialysis | text_dialysis_mentions"`, or `"a & !b"`), and `NA` for a single-channel variable — not the kind. The remaining target parts are still future slices; until they land the running engine keeps the older surface (`decision`/`decision_state`, public `role`, the now-inert `absence_policy=` knob, and the `binary_/number_/categorical_/fields_output` names), and the §6 / §16 examples still show that shipped spelling.

- **`combine` = hit-algebra over channel hit sets, and nothing else** (produces `0`/`1`). `any_positive()` is **syntax sugar** that lowers to a raw expression: `any_positive(a, b)` → stored `combine_rule = "a | b"` (not `"any_positive"`, not `"hit_set_expr"`). Shipped as of slice 3.
- **`documented_status()` and `collect_fields()` are NOT combines.** They are single-channel **output assembly** (categorical passthrough / field fan-out), like `max_value`. They run with `combine = NULL` and are dispatched by `output` shape; for them `combine_rule = NA`. (`documented_status` was a trivial passthrough and is gone as a named operator; the categorical assembler remains.) **Slice 2 made `output` the single-channel dispatch key** (the runner previously dispatched only on `combine$kind` and never read `output`).
...
**Migration slices:** (1) this note ✅ · (2) ✅ **[done, non-transitional]** promoted `output` to the dispatch key + `documented_status`/`collect_fields` → output assembly, **absorbing** the `any_positive`→raw-expression lowering and the template `combine`-removal so `is.null(combine)` holds directly (no legacy-combiner tolerance); a new single-channel `bin_output` membership assembler was added, and `channel_coverage` gained `failed` (a channel errored) distinct from `partial` for the lowered OR path · (3) ✅ `combine_rule` = raw expression (NA for a single-channel variable) · (4) drop public `role`/`polarity` · (5) drop `decision`/`decision_state` · (6) normalise `processing_state` + unify `channel_coverage` · (7) reconcile/remove the now-inert `absence_policy` (the `any_positive` lowering moved into slice 2; `combine_any_channel_hit` + its `test-multisource.R` tests are already deleted, so `incomplete_value` is read by nothing) · (8) `hit_*` → `match_*` · (9) `answer_spec` / `method` / output-type renames.
